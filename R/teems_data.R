...
#'
#' @description `teems_data()` loads coefficient data including base data,
#'   parameters, and any auxillary data as well as conducts pre-pipleine checks.
#'   The output of this function is a required input to the `"data_config"`
...
#'   * Equivalent list object generated by [`GTAP_convert()`].
#' @param par_input Input containing parameter coefficient data. Character of
#'   length 1, file name in working directory or path to a GTAP parameter "par"
#'   HAR file.
#'   * Equivalent list object generated by [`GTAP_convert()`].
#' @param aux_input Input containing auxillary coefficient data. Currently used
#'   primarily to load time related data for dynamic models.
#'   * Character of length 1, file name in working directory or path
#'   to a GTAP HAR file.
#'   * Equivalent list object generated by [`GTAP_convert()`] or a helper
#' function such as [`teems_time()`].
#' @param unaggregated_input A named list (default is `NULL`). Key-value pairs
#'   denoting the name of the header to replace and path to a CSV file or data
#'   frame or data frame extension containing data. User-provided data must
#'   contain all corresponding sets and associated elements present in the
#'   original header data as well as a "Value" column with new values. This
#'   replacement will be subject to structure checks and set aggregations.
#' @param aggregated_input A named list (default is `NULL`). Key-value pairs
#'   denoting the name of the header to replace and path to a CSV file or data
#'   frame or data frame extension containing data. A length 1 numeric may be
#'   provided for coefficents with no set affiliations (e.g., binary switches).
#'   User-provided data must contain all set aggregation-specific elements to be
#'   replaced as well as a "Value" column with new values. This replacement will
#'   be subject to structure checks according to selected set aggregations.
#'
#' @return A list of data configuration options.
#'
#' @details `teems_data()` return values have no purpose used in isolation and
...
#'
#' @seealso [`GTAP_convert()`] for converting from HAR file type into a list as 
