...
process_mappings <- function(path,
  ...
  ls_mappings <- list()
  for (v in db_version) {
    set_names <- list.files(path = file.path("data-raw", "mappings", v))
    for (s in set_names) {
      set_mappings <- list.files(path = file.path("data-raw", "mappings", v, s),
                                 recursive = TRUE,
                                 full.names = TRUE)

      # Read the files into a list of data frames and rename columns
      dt_list <- lapply(X = set_mappings,
                        FUN = function(file) {
                          ...
                          data.table::setnames(x = dt, old = "mapping", new = mapping_name)

                          return(dt)
                        })

      # Merge all data frames in the list by the first column
      merged_dt <- Reduce(f = function(x, y) {merge(x, y, by = colnames(x = dt_list[[1]])[1], all = TRUE)},
                          x = dt_list)

      ls_mappings[[v]][[s]] <- merged_dt
    }
  }
